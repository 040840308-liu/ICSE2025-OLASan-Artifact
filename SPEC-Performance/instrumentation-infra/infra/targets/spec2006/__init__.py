    ...
    def build(self, ctx, instance, pool=None):
        ...
        for bench in self._get_benchmarks(ctx, instance):
            cmd = 'killwrap_tree runspec --config=%s --action=build %s' % \
                  (config, bench)
            ctx.log.info(f"build-wxl {bench}")
            #os.environ['SPECNAME'] = bench
            os.environ['SPECNAME'] = bench
            #print(bench)
            #print(os.environ['SPECNAME'])
            result = subprocess.run(['bash', '-c', 'echo $SPECNAME'], check=True, stdout=subprocess.PIPE, env=os.environ, text=True)
            result = subprocess.run(['bash', '-c', 'source ~/.bashrc'], check=True, stdout=subprocess.PIPE, env=os.environ, text=True)
            if pool:
